...
# ===========
SEED = 42
EXPERT_TRAIN_PATH = 'gcGAILDateset_train.csv'
TEST_PATH = 'gcGAILDateset_test.csv'
...
OUT_DIR = pathlib.Path(f"outputs/gcGAIL_{TS}")
...
def load_expert_data(file_path: str) -> Dict[Any, List[Tuple[np.ndarray, np.ndarray, np.ndarray, float, bool]]]:
    ...
    for expert_id, group in df.groupby('PassengerID'):
        states = group[['HomeLoc', 'WorkLoc','WorkLocIsDiscSta', 'Months_Passed', 'LastMonthMode', 'TwoMonthMode',
                        'LastTripStart', 'LastTripEnd', 'LastTripFare', 'LastMinShift',
                        'MeanMorningPeakMoneySave', 'FirstWeekdayMorningTapInTimeStd',
                        'AvgTripTravelTime','MeanTimeShift']].values.astype(np.float32)

        home_loc = states[:, 0].astype(int)
        work_loc = states[:, 1].astype(int)
        work_loc_is_disc_sta = states[:, 2].astype(int)

        home_loc_one_hot = np.eye(home_loc_dim)[home_loc]
        work_loc_one_hot = np.eye(work_loc_dim)[work_loc]
        work_loc_is_disc_sta_one_hot = np.eye(work_loc_is_disc_sta_dim)[work_loc_is_disc_sta]

        # normalize continuous columns (3:)
        for i, col_name in enumerate(normalization_ranges.keys()):
            min_val, max_val = normalization_ranges[col_name]
            states[:, i + 3] = (states[:, i + 3] - min_val) / (max_val - min_val)
            states[:, i + 3] = np.clip(states[:, i + 3], 0, 1)

        states = np.concatenate([home_loc_one_hot, work_loc_one_hot, work_loc_is_disc_sta_one_hot, states[:, 3:]], axis=1)

        # conditions
        conditions = group[['con_flex', 'con_dis', 'con_con']].values.astype(np.float32)
        con_flex = conditions[:, 0].astype(int)
        con_dis = conditions[:, 1].astype(int)
        con_con = conditions[:, 2].astype(int)

        con_flex_one_hot = np.eye(con_flex_dim)[con_flex - 1]
        con_dis_one_hot  = np.eye(con_dis_dim)[con_dis - 1]
        con_con_one_hot  = np.eye(con_con_dim)[con_con - 1]

        obs = np.concatenate([states, con_flex_one_hot, con_dis_one_hot, con_con_one_hot], axis=1)

        actions = group[['MonthMode']].values
        next_obs = np.roll(obs, -1, axis=0)
        next_obs[-1] = np.zeros(obs.shape[1])

        rewards = group['LastTripFare'].values
        dones = group['done'].values.astype(bool).tolist()

        trajectory = list(zip(obs, actions, next_obs, rewards, dones))
...
class CustomEnv(gym.Env):
    """Returns imitation reward 1/0 (action equals expert). GAIL wraps & replaces reward when training."""
    metadata = {"render_modes": []}

    def __init__(self, expert_data: Dict[Any, Any]):
        super().__init__()
        self.expert_data = expert_data
        self.expert_ids = list(expert_data.keys())

        self.action_space = spaces.Discrete(2)
        self.num_categorical = [121, 121, 2, 4, 4, 4]  # home, work, work_is_disc, con_flex, con_dis, con_con
        self.continuous_dim = 11
        self.flat_observation_size = self.continuous_dim + sum(self.num_categorical)
    ...
    def step(self, action):
        ...
        return obs, float(imitation_reward), done, False, {}
...
def sample_random_params() -> Dict[str, Any]:
    return {
        ...
        "batch_size": np.random.choice([128, 256]),
        "n_steps": np.random.choice([1024, 2048]),
...
def main():
    ...
    # Initial Policy defaults (can be overridden by search)
    tuning_params = {
        "learning_rate": 1.7745172999880464e-05,
        "gamma": 0.95,
        "batch_size": 128,
        "n_epochs": 20,
        "n_steps": 1024,
    ...
    gen_batch_size = learner.n_steps * venv.num_envs
    eval_stride_rounds = max(1, EVAL_INTERVAL // gen_batch_size)
    ...
    print(">>> Training gcGAIL (full rounds)...")
    ...
    for r in range(n_rounds):
        gail_trainer.train(gen_batch_size)
        steps_done += gen_batch_size

        # logging cadence
        if (r + 1) % eval_stride_rounds == 0 or r == n_rounds - 1:
            logs = dict(gail_trainer.logger.name_to_value or {})
    ...
    runtime["gail_train_sec"] = time.perf_counter() - t_train0
    ...
    # Save model
    model_path = MODELS_DIR / "ppo_gcgail.zip"
    ...
    try:
        ...
        df_test['gcGAIL_pred'] = all_pred
        df_test.to_csv(OUT_DIR / "Prediction_with_gcGAIL.csv", index=False)
    ...
    with open(REPORTS_DIR / "summary.txt", "w", encoding="utf-8") as f:
        f.write("==== gcGAIL Training Summary ====\n")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="gcGAIL training pipeline (round-safe CLI)")
