...
OUT_DIR = pathlib.Path(f"outputs/GAIL_{TS}")
...
def load_expert_data(file_path: str) -> Dict[Any, List[Tuple[np.ndarray, np.ndarray, np.ndarray, float, bool]]]:
    ...
    # Define the number of unique categories for one-hot encoding
    home_loc_dim = 121  # Assuming HomeLoc has 122 unique values
    work_loc_dim = 121  # Assuming WorkLoc has 122 unique values
    work_loc_is_disc_sta_dim = 2  # Binary variable (0 or 1)
    ...
    for expert_id, group in df.groupby('PassengerID'):
        states = group[
            ['HomeLoc', 'WorkLoc', 'WorkLocIsDiscSta', 'Months_Passed', 'LastMonthMode', 'TwoMonthMode',
             ...
             'MeanMorningPeakMoneySave']].values.astype(np.float32)
...
class CustomEnv(gym.Env):
    ...
    def __init__(self, expert_data):
        ...
        self.continuous_dim = 8 # Number of continuous variables
...
def main():
    ...
    # Save model
    model_path = MODELS_DIR / "ppo_gail.zip"
    ...
    try:
        ...
        df_test['GAIL_pred'] = all_pred
        df_test.to_csv(OUT_DIR / "Prediction_with_GAIL.csv", index=False)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="GAIL training pipeline (round-safe CLI)")
