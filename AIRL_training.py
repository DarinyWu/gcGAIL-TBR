...
def plot_series(
    x, ys: Dict[str, List[float]], title: str, out_path: pathlib.Path, xlabel="step", ylabel="value"
):
    ...
    for expert_id, group in df.groupby("PassengerID"):
        states = group[
            [
                ...
                "LastTripFare",
                "LastMinShift",
                "MeanMorningPeakMoneySave",
                "FirstWeekdayMorningTapInTimeStd",
                "AvgTripTravelTime",
                "MeanTimeShift",
            ]
        ...
        states = np.concatenate(
            [home_loc_one_hot, work_loc_one_hot, work_loc_is_disc_sta_one_hot, states[:, 3:]], axis=1
        )

        actions = group[["MonthMode"]].values

        next_states = np.roll(states, -1, axis=0)
        next_states[-1] = np.zeros(states.shape[1])
...
class CustomEnv(gym.Env):
    """A custom environment for AIRL behavior imitation (expert trajectory replay)."""
    ...
    def step(self, action):
        ...
        # imitation reward: 1 if action matches expert, else 0
...
def hyperparam_search(expert_transitions, expert_data) -> Dict[str, Any]:
    ...
    payload = {
        "best_mean_reward": float(best_mean),
...
def main():
    ...
    # PPO defaults (can be overridden by search)
    tuning_params = {
    ...
    for r in range(n_rounds):
        ...
        if (r + 1) % eval_stride_rounds == 0 or r == n_rounds - 1:
            ...
            for k_src, k_dst in [
                ("discriminator/accuracy", "disc/accuracy"),
            ...
            print(f"[{steps_done}/{target_steps}] mean_return={mean_r:.3f}, disc_acc={metrics['disc/accuracy'][-1]}")

            # Early stopping (return-based)
            ...
            # Early stopping (disc acc near 0.5)
    ...
    if steps_x:
        ...
        plot_series(
            ...
            "Discriminator metrics vs. steps",
            PLOTS_DIR / "discriminator_curves.png",
        )
    ...
    hist_plot({"expert": r_exp, "generator": r_gen}, "Learned reward distribution", PLOTS_DIR / "reward_histograms.png")
    ...
    np.savez(
        OUT_DIR / "returns_before_after.npz",
    ...
    print(
        f"Mean return before: {np.mean(rewards_before):.3f} | after: {np.mean(rewards_after):.3f} "
