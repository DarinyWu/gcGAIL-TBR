...
OUT_DIR = pathlib.Path(f"outputs/cGAIL_{TS}")
...
def main():
    ...
    # Save model
    model_path = MODELS_DIR / "ppo_cgail.zip"
    ...
    try:
        ...
        df_test['cGAIL_pred'] = all_pred
        df_test.to_csv(OUT_DIR / "Prediction_with_cGAIL.csv", index=False)
    ...
    with open(REPORTS_DIR / "summary.txt", "w", encoding="utf-8") as f:
        f.write("==== cGAIL Training Summary ====\n")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="cGAIL training pipeline (round-safe CLI)")
